...
# Plot signal X
ch_info = create_info(ch_names=ch_names, ch_types=["eeg"] * ch_count, sfreq=sfreq)
...
plot_cospectra(
    ajdc._cosp_channels, freqs, ylabels=ch_names, title="Cospectra, in channel space"
)
...
# Plot sources S
sr_info = create_info(ch_names=sr_names, ch_types=["misc"] * sr_count, sfreq=sfreq)
...
f, spectrum = welch(
    source.get_data(picks=[blink_idx]),
    fs=sfreq,
    nperseg=window,
...
axs[0].set(
    title="Power spectrum of the blink source estimated by AJDC",
    xlabel="Frequency (Hz)",
...
signal_denois_raw = ajdc.inverse_transform(
    source_raw[np.newaxis, ...], supp=[blink_idx]
)[0]
