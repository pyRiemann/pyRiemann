...
class TransferTestCase:
    def test_transfer_with_target_sample(self, transfer,
        ...
        n_matrices_class_source, n_matrices_class_target = 3, 2
        n_matrices_source = n_classes * n_matrices_class_source
        n_matrices_target = n_classes * n_matrices_class_target
        sample_weight = None

        covmats_source = get_covmats(n_matrices_source, n_channels)
        labels_source = get_labels(n_matrices_source, n_classes)
...
def test_different_labels_error(transfer, get_covmats, get_labels):
    with pytest.raises((ValueError)):
        n_classes, n_classes_source, n_channels = 6, 4, 3
        n_matrices_class_source, n_matrices_class_target = 3, 2
        n_matrices_source = n_classes_source * n_matrices_class_source
        n_matrices_target = n_classes * n_matrices_class_target
        sample_weight = None

        covmats_source = get_covmats(n_matrices_source, n_channels)
        labels_source = get_labels(n_matrices_source, n_classes_source)
...
def test_sample_weight_shape_error(transfer, excess, get_covmats, get_labels):
    with pytest.raises((ValueError)):
        n_classes, n_channels = 4, 3
        n_matrices_class_source, n_matrices_class_target = 3, 2
        n_matrices_source = n_classes * n_matrices_class_source
        n_matrices_target = n_classes * n_matrices_class_target
        sample_weight = np.ones(n_matrices_source + excess)
...
def test_n_channels_mismatch_error(transfer, excess, get_covmats, get_labels):
    with pytest.raises((ValueError)):
        ...
        n_matrices_source = n_classes * n_matrices_class_source
        n_matrices_target = n_classes * n_matrices_class_target
        sample_weight = np.ones(n_matrices_source)

        covmats_source = get_covmats(n_matrices_source, n_channels + excess)
        labels_source = get_labels(n_matrices_source, n_classes)
...
def test_target_matrices_mismatch(transfer, excess, get_covmats, get_labels):
    with pytest.raises((ValueError)):
        ...
        covmats_target = get_covmats(n_matrices_target, n_channels)
        labels_target = get_labels(n_matrices_target + excess, n_classes)
...
def test_source_matrices_mismatch(transfer, excess_X, excess_y, excess_weight,
    ...
    with pytest.raises((ValueError)):
        ...
        n_matrices_source = n_classes * n_matrices_class_source
        n_matrices_target = n_classes * n_matrices_class_target
        sample_weight = np.ones(n_matrices_source + excess_weight)

        covmats_source = get_covmats(n_matrices_source + excess_X, n_channels)
        labels_source = get_labels((n_matrices_source + excess_y), n_classes)
